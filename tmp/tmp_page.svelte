<script lang="ts">
	...
	import HD from '$lib/components/htmlDialog.svelte'

	let htmldial = $state<HTMLDialogElement>();
	let dialisopen = $state(false);
</script>

<button class="rounded-input bg-blue-700 text-background mx-4 py-[10px] px-[12px]"
onclick={() => htmldial?.showModal()}> 
Открыть диалог 
</button>

<HD bind:dial={htmldial}>
	<p>
...
<div>
	<label>
<input type="checkbox" bind:checked={dialisopen} />
Dialog
</label>
</div>
<div>
	<Dialog.Root open={dialisopen}>
		<Dialog.Trigger
